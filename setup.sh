...
echo "  Champollion / Caprica  -- Papyrus decompile / compile"
echo "  Spriggit               -- ESP <-> YAML editing (dotnet tool)"
echo "  xeditlib               -- programmatic ESP read/write (Node.js + XEditLib.dll)"
echo "  AutoMod CLI            -- NIF / BSA / audio / MCM / ESP (use tools/automod-cli.sh)"
echo "  PyFFI (Python 3.10)    -- LE-format NiTriShape geometry edits"
echo "  PyNifly                -- SSE BSTriShape + animation/controller authoring + NIF parse gate"
echo "  Blender (headless)     -- NIF mesh repair + render-to-PNG (tools/blender-nif-*.py; needs the"
echo "                            PyNifly Blender addon)"
echo "  NifSkope               -- independent visual NIF validator (GUI)"
echo ""
